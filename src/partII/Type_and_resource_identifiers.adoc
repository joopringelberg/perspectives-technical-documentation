...
In model://perspectives.domains#CouchdbManagement, both a Repository and a ModelManifest have a public role. In fact, the ModelManifest contextrole property `LocalModelName` falls in both public perspectives. Either perspective publishes to a different location. Where does the contextrole of ModelManifest end up?
...
One approach to the problem posed above, therefore, is just to publish the contextrole of ModelManifest and its property `LocalModelName` to _both locations_. Consider for a moment however, what would happen if a visitor of Repository opens a ModelManifest instance. Doing so he assumes the ModelManifest$Visitor role, which gives him access to, a.o., the Versions role. But as Repository$Visitor does not have a perspective on that role, it is not represented at the particular url where Repository$Visitor has published. No Versions will turn up! In contrast, were the user to visit ModelManifest directly, he would find instances of Versions. Clearly this is not what we want.

For that reason we have adopted another strategy. We define, for each context, an optional public url. It will be available if there is a public user role in that context. Now, for each role instance, if its context does have a public url, we will publish it there. If it has no public url of itself (and we are indeed publishing) we use the public url of the user we are actually publishing for.

Let's consider some examples. 

In case of the role ModelManifest$External (with property LocalModelName), we find that ModelManifest does have a public url - it is the url where its Visitor role publishes to. So, even if we're in the middle of publishing _for_ Repository$Visitor, we'll publish ModelManifest$External to the url of ModelManifest$Visitor rather than to Repository$Visitor's url.
